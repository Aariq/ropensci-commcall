...
## Running targets in parallell

-   If you set up your workflow with `use_targets()` then you're likely already in good shape.

-   E.g. `tar_make_clustermq(workers = 3)` or `tar_make_future(workers = 3)`
...
-   Overhead: One-time cost to set up workers
...
-   Cluster setup doesn't solve problem of working from the comfort of RStudio

-   Cluster use through command line or X11 window RStudio (awful quality)

-   Targets store not synced to cluster via GitHub
...
-   UA provides Open On Demand RStudio in web browser
    -   `use_targets()` detects SLURM scheduler, but Open On Demand session can't submit SLURM jobs
    -   Doesn't solve the problem of syncing the targets store, but makes it easier to develop on the cluster
:::
